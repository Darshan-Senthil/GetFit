...
// Muscle groups with MuscleWiki API IDs (same as workouts)
const muscleGroups = [
...
export type StretchMuscleGroup = (typeof muscleGroups)[number];
export type StretchMuscleGroupId = StretchMuscleGroup["id"];

interface StretchMuscleSelectorProps {
  selected: StretchMuscleGroup | null;
  onSelect: (group: StretchMuscleGroup) => void;
}
...
export function StretchMuscleSelector({
  selected,
  onSelect,
}: StretchMuscleSelectorProps) {
  return (
          ...
          return (
            <button
              ...
              className={cn(
                ...
                "border outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/50",
                isSelected
                  ? "bg-cyan-500/20 border-cyan-500/50 text-cyan-400 shadow-lg shadow-cyan-500/10"
                  : "bg-card/50 border-border hover:border-cyan-500/30 text-muted-foreground hover:text-foreground hover:bg-card"
              )}
            >
              <Icon
                className={cn(
